      ...
      if (response.statusCode == 200) {
        ...
        if (UpdateService.isVersionNewerStatic(currentVersion, latestVersion)) {
          ...
          for (final asset in assets) {
            if (asset['name'] is String && (asset['name'] as String).endsWith('.apk')) {
        ...
        setState(() {
          _errorMessage = 'Impossibile controllare gli aggiornamenti (errore ${response.statusCode})';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _downloadAndInstall() async {
    ...
    if (apkUrl == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('URL APK non disponibile')),
      );
      return;
    }
    ...
    // Richiedi permesso di scrittura storage
    if (!await Permission.storage.isGranted) {
      final status = await Permission.storage.request();
      if (!status.isGranted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Permesso storage negato')),
        );
        return;
      }
    }
